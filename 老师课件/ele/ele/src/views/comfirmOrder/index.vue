    ...
    <div class="page">
       <my-nav-bar>
        ...
        <div>
            ...
            <van-contact-card type="edit" :name="ArrayList[0].name+' '+ArrayList[0].phone" :tel="ArrayList[0].address" v-else/>
        </div>
        <div class="sdsj" >
            <div class="time">
                送达时间
            </div>
            <div class="right" v-if="orderinfo">
        ...
        <div class="foods"  v-if="orderinfo">
            <div class="foodsname">
            ...
            <div>
                <div class="foodslist" v-for="(item,index) in orderinfo.cart.groups" :key="index">
                    <span>
                        {{item[0].name}} 
                    </span>
                    <span>
                        <span class="num"> ✖ {{item[0].quantity}}</span>
        ...
        <div class="footer" v-if="orderinfo">
            ...
            <div class="btn" @click="getOrder">确认下单</div>
        </div>
...
<script>
import {createOrder,getAddressList,orderDon} from '@/api/order'
...
export default {
    ...
    //   获取当前用户收获地址
      let address =  await getAddressList(this.$store.state.user_id);
      console.log(address)
      this.ArrayList = address.data;
    },
    methods:{
        // 确认下单
        async getOrder(){
            let user_id = this.$store.state.user_id;
            let cart_id = this.orderinfo.id;
            if(this.ArrayList.length == 0){
                Toast.fail("请您选择收获地址")
                return
            }
            let data = {
                address_id:this.ArrayList[0].id,
                restaurant_id:this.$store.state.id,
                geohash:localStorage.getItem("geohash"),
                description:"",
            ...
            console.log(data)
            let res = await orderDon(user_id,cart_id,data);
